...
@onready var _label_extends: Label = %LabelExtends
@onready var _rich_text_extends: RichTextLabel = %RichTextExtends
...
func _set_label_name() -> void:
	...
	_label_extends.text = __c._setup_settings._text_dict["e_info"]
	_label_extends.add_theme_font_size_override("font_size", _font_size)
	__c._setup_settings._set_theme_override_font_color(_label_extends, "font_color", _color)

func _set_rich_label_name() -> void:
	var _font_size: int = min(__c._setup_project._get_popup_text_size(), 24)
	_rich_text_extends.text = __c._setup_settings._text_dict["init"]
	_rich_text_extends.add_theme_font_size_override("bold_font_size", _font_size)
	_set_font_color()
...
func _set_rich_text_insert(_text: String) -> void:
	_rich_text_extends.text = "%s" % _get_bbcode_text(_text, "bold")
	_set_font_color()

func _set_font_color() -> void:
	var _color: Color = __c._setup_settings._color_dict["base_color"]
	__c._setup_settings._set_rich_label_default_color(_rich_text_extends, _color * 1.05)
...
func _set_init_rich_name() -> void:
	_rich_text_extends.text = __c._setup_settings._text_dict["init"]
	_rich_text_extends.tooltip_text = ""
	_set_font_color()
